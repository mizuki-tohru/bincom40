...
//#define LINGBUF_SIZE 32768 //0x8000
...
// 追加: ハンドル有効判定と安全クローズ
static inline bool IsHandleValid(HANDLE h) {
...
static void LogOverlappedError(const char *where, DWORD err)
{
    LPWSTR msgBuf = NULL;
    FormatMessageW(
    ...
    if (msgBuf) {
        ...
        wchar_t out[128];
        swprintf_s(out, _countof(out), L"[Overlapped] %S: 0x%08X\n", where, err);
        OutputDebugStringW(out);
    }
}
...
char TMPDMY[100];
int frm_U[RTN_SIZE];			//画面表示ASCII-UTF-8-HEXフラグ
int sfrm_U[RTN_SIZE];			//画面表示ASCII-UTF-8-HEXフラグ
int retcnt;
...
// ヘルパ: 安全な frmm アクセス
static inline unsigned long frmm_idx_from_ulong(unsigned long p) noexcept {
	return p & (unsigned long)LINGBUF_MAX;
}
static inline unsigned long frmm_idx_from_long(long p) noexcept {
	return ((unsigned long)p) & (unsigned long)LINGBUF_MAX;
}
static inline unsigned long prev_frmm_idx(unsigned long p) noexcept {
	return (p + LINGBUF_SIZE - 1u) & (unsigned long)LINGBUF_MAX;
}
static inline const char* FRMM_ptr(unsigned long p) noexcept {
	return &frmm[frmm_idx_from_ulong(p)];
}
static inline const char* FRMM_ptr_long(long p) noexcept {
	return &frmm[frmm_idx_from_long(p)];
}
static inline char FRMM_at(unsigned long p) noexcept {
	return frmm[frmm_idx_from_ulong(p)];
}
static inline char FRMM_at_long(long p) noexcept {
	return frmm[frmm_idx_from_long(p)];
}
static inline void FRMM_set(unsigned long p, char v) noexcept {
	frmm[frmm_idx_from_ulong(p)] = v;
}
static inline void FRMM_set_long(long p, char v) noexcept {
	frmm[frmm_idx_from_long(p)] = v;
}
static inline long get_RtnPos_safe(int idx) noexcept {
	if (idx < 0 || idx >= RTN_SIZE) return -1;
	long v = RtnPos[idx & 0xff];
	if (v < 0) return -1;
	if (((unsigned long)v) >= LINGBUF_SIZE) return -1;
	return v;
}
...
// UHDEV コンテナと保護用ミューテックス
//static std::mutex UHDEV_mtx;
//static std::vector<UsbhidDev> UHDEV;
std::mutex UHDEV_mtx;
std::vector<UsbhidDev> UHDEV;
...
class UniqueHandle {
public:
    UniqueHandle(HANDLE h = NULL) noexcept : h_(h) {}
    ~UniqueHandle() { if (h_ && h_ != INVALID_HANDLE_VALUE) CloseHandle(h_); }
    UniqueHandle(UniqueHandle&& o) noexcept : h_(o.h_) { o.h_ = NULL; }
    UniqueHandle& operator=(UniqueHandle&& o) noexcept { reset(); h_ = o.h_; o.h_ = NULL; return *this; }
    // no copy
    UniqueHandle(const UniqueHandle&) = delete;
    UniqueHandle& operator=(const UniqueHandle&) = delete;
    HANDLE get() const noexcept { return h_; }
    HANDLE release() noexcept { HANDLE t = h_; h_ = NULL; return t; }
    void reset(HANDLE h = NULL) noexcept { if (h_ && h_ != INVALID_HANDLE_VALUE) CloseHandle(h_); h_ = h; }
    explicit operator bool() const noexcept { return h_ && h_ != INVALID_HANDLE_VALUE; }
private:
    HANDLE h_;
};

// RAII for GDI objects (HFONT 等)
class UniqueGdi {
public:
    UniqueGdi(HGDIOBJ o = NULL) noexcept : o_(o) {}
    ~UniqueGdi() { if (o_) DeleteObject(o_); }
    UniqueGdi(UniqueGdi&& o) noexcept : o_(o.o_) { o.o_ = NULL; }
    UniqueGdi& operator=(UniqueGdi&& o) noexcept { reset(); o_ = o.o_; o.o_ = NULL; return *this; }
    UniqueGdi(const UniqueGdi&) = delete;
    UniqueGdi& operator=(const UniqueGdi&) = delete;
    HGDIOBJ get() const noexcept { return o_; }
    void reset(HGDIOBJ o = NULL) noexcept { if (o_) DeleteObject(o_); o_ = o; }
    explicit operator bool() const noexcept { return o_ != NULL; }
private:
    HGDIOBJ o_;
};
...
static inline std::unique_ptr<void, DevInfoCloser> make_devinfo(HDEVINFO h) noexcept {
    return std::unique_ptr<void, DevInfoCloser>(reinterpret_cast<void*>(h));
}
//-------------
static void RegWriteSafe(size_t regIndex, long addr, long value) {
    if (regIndex >= REG.size()) {
        // ログ出力（必要なら OutputDebugString を使う）
        return;
    }
    if (REG[regIndex].isize <= 0) return;
//  if (REG[regIndex].DATA == nullptr) return;
    if (REG[regIndex].DATA.size() == 0) return;
    if (addr < 0 || addr >= REG[regIndex].isize) {
        // 範囲外アクセスを検出した場合はログを残す
        char buf[128];
        sprintf_s(buf, sizeof(buf), 
        		"RegWriteSafe: out of range reg=%zu addr=%ld size=%d\n",
        		regIndex, addr, REG[regIndex].isize);
        OutputDebugStringA(buf);
        return;
    }
    REG[regIndex].DATA[addr] = value;
}
...
void scroll(unsigned int kai)
{
    ...
    for(j=0;j<kai;j++){
		if(frm_U[FstN&0xff] == 1){//Hex
			...
			if(FstN < RtnN){
				FstN++;
				RtnPos[(FstN-1)&0xff] = -1;
				RtnPos[FstN&0xff] = stpoint;
			}
		}
	}
	if((RtnPos[FstN&0xff]&LINGBUF_MAX)<(RtnPos[RtnN&0xff]&LINGBUF_MAX)){
		if(((RtnN&0xff) - (FstN&0xff)) < (LINGBUF_SIZE/2)){
			FstN = FstN&0xff;
			RtnN = RtnN&0xff;
			RtnPos[FstN&0xff] = RtnPos[FstN&0xff]&LINGBUF_MAX;
			RtnPos[RtnN&0xff] = RtnPos[RtnN&0xff]&LINGBUF_MAX;
			stpoint = RtnPos[FstN&0xff];
			edpoint = RtnPos[RtnN&0xff];
		}
	}
}
//------------------------------------------------------------------------
...
void send_scroll(unsigned int kai)
{
    ...
    for(j=0;j<kai;j++){
		if(sfrm_U[sRtnN&0xff] == 1){//Hex
			if(sRtnPos[(sFstN+1)&0xff] > 0){
				if(sRtnN > sFstN){
					sFstN++;
					sRtnPos[(sFstN-1)&0xff] = -1;
				}
			}
			sstpoint = sRtnPos[sFstN&0xff];
		}else{
	    	while((sfrmm[sstpoint&SENDBUF_MAX]!=0)
	    	    &&(sfrmm[sstpoint&SENDBUF_MAX]!='\n')){//表示先頭の行終端を探す
				sstpoint++;
			}
			//ここでstpointは0か\nを指している
			while(sfrmm[sstpoint&SENDBUF_MAX] == 0){
			...
			sFstN++;
			sRtnPos[(sFstN-1)&0xff] = -1;
			sfrm_U[(sFstN-1)&0xff] = 1;//-----------
			sRtnPos[sFstN&0xff] = sstpoint;
		}
	}
	if((sRtnPos[sFstN&0xff]&SENDBUF_MAX)<(sRtnPos[sRtnN&0xff]&SENDBUF_MAX)){
		if(((sRtnN&0xff) - (sFstN&0xff)) < (SENDBUF_SIZE/2)){
			sRtnPos[sFstN&0xff] = sRtnPos[sFstN&0xff]&SENDBUF_MAX;
			sRtnPos[sRtnN&0xff] = sRtnPos[sRtnN&0xff]&SENDBUF_MAX;
		}
	}
}
/*-------------------------------------------------------------------------------------*/
...
void DisplayData(HWND hWnd, unsigned char* Bufin, DWORD len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if ((Bufin[i] == '\n')||(Bufin[i] == '\r')) {
			if(edpoint!=0){
				if(FRMM_at(prev_frmm_idx(edpoint)) != '\n'){//CRLF除去
					FRMM_set(edpoint, '\n');
					if((edpoint+1) == stpoint){
						stpoint++;
						if((FRMM_at(stpoint)!=0) && (FRMM_at(stpoint)!='\n')) stpoint++;
						//stpointを先行させ直す
					}
					retcnt++;
				}//既に改行文字がある場合文字は無視される
			...
			m = UTF8Len((const unsigned char *)FRMM_ptr(lnpoint), ws, &n);
			if(m == (int)strlen(FRMM_ptr(lnpoint))){//ASCII
			//	frm_U[k&0xff] = 0;
				frm_U[RtnN&0xff] = (Display_flg & 0x01);
			}else{
				frm_U[RtnN&0xff] = 2;
			}
			if(m>ws){//一行より大きいなら改行する
				for(k=edpoint;k>(lnpoint+n);k--){
					FRMM_set(k+1, FRMM_at(k));
				}
				FRMM_set(lnpoint+n+1, 0);
				edpoint++;
				retcnt++;
			...
			edpoint++;
			FRMM_set(edpoint, 0);//行末
			if(edpoint == stpoint){
				stpoint++;
				if((frmm[stpoint&LINGBUF_MAX]!=0)
				 &&(frmm[stpoint&LINGBUF_MAX]!='\n'))stpoint++;
			}
			m = UTF8Len((const unsigned char *)&frmm[lnpoint&LINGBUF_MAX],ws,&n);
			...
			edpoint++;
			FRMM_set(edpoint, 0);
			if(edpoint+1 < LINGBUF_SIZE) FRMM_set(edpoint+1, 0);
			if(edpoint+2 < LINGBUF_SIZE) FRMM_set(edpoint+2, 0);
			if(edpoint+3 < LINGBUF_SIZE) FRMM_set(edpoint+3, 0);
			if(edpoint == stpoint){
				stpoint++;
				if((frmm[stpoint&LINGBUF_MAX]!=0)
				 &&(frmm[stpoint&LINGBUF_MAX]!='\n'))stpoint++;
			...
			if((long)n >= ws){
				...
				FRMM_set(lnpoint+n+1, 0);
				RtnPos[RtnN&0xff] = lnpoint+n+1;/*書き直し*/
				edpoint++;
				retcnt++;
				...
				RtnPos[RtnN&0xff] = lnpoint+n+2;//新しい終端
				lnpoint = lnpoint+n+2;
			}
			if(retcnt >= ((hs*4)+(3-wh)-1)) {
				scroll(1);			//収まるようにスクロール
...
void SendDataP(HWND hWnd,unsigned char * Bufin,DWORD len)
{
	...
	ws = ((WSIZE_WIDTH-12)/8);
	ds = (ws/3)-1;
	hs = ((WSIZE_HEIGHT-80)/75)-1;
	unsigned int cv = (((WSIZE_WIDTH-12)/8)-2);//64
	unsigned int ct = (((WSIZE_WIDTH-12)/24)-2);//20
	int wh = FIXParam_Cnt/(int)((WSIZE_WIDTH-2)/135);
	...
	if(len > 0){
		for(i=0;i<len;i++){
			sfrmm[sedpoint&SENDBUF_MAX] = Bufin[i];
			sedpoint++;//行末位置更新
		}
		sfrmm[sedpoint&SENDBUF_MAX] = 0;
	//	if(((sedpoint/ct)>hs)||(sRtnN>hs))send_scroll((len/ct)+1);
		if(((sedpoint/ct)>(hs-1))||(sRtnN>(hs-1)))send_scroll((len/ct)+1);
		sRtnN++;
		if(sRtnN == sFstN){
			send_scroll(1);
			sRtnPos[sFstN&0xff] = sstpoint;
		...
		sfrm_U[sRtnN&0xff] = 1;
	//	sfrm_U[sRtnN&0xff] = sfrm_U[(sRtnN-1)&0xff];
	}else{
		o = (sRtnPos[sRtnN&0xff] - sRtnPos[(sRtnN-1)&0xff]);
		if(o>0)o--;
		for(i=0;i<o;i++){//前回入力を表示する
			sfrmm[sedpoint&SENDBUF_MAX] = sfrmm[(sRtnPos[(sRtnN-1)&0xff]+i)&SENDBUF_MAX];
			sedpoint++;//行末位置更新
		}
		sfrmm[sedpoint&SENDBUF_MAX] = 0;
	//	if(((sedpoint/ct)>hs)||(sRtnN>hs))send_scroll((o/ct)+1);
		if(((sedpoint/ct)>(hs-1))||(sRtnN>(hs-1)))send_scroll((o/ct)+1);
		sRtnN++;
		if(sRtnN == sFstN){
			send_scroll(1);
			sRtnPos[sFstN&0xff] = sstpoint;
		...
		if(sRtnPos[j&0xff]!=-1){
			...
			if(sfrm_U[j&0xff] == 1){
				...
				for(i=sRtnPos[j&0xff];i<(sRtnPos[j&0xff]+p);i++){
    				ltohex((unsigned char)(sfrmm[i&SENDBUF_MAX]),&buf[(m*3)],2);
			...
			TextOutW(hdc,4,((((hs+1)*4)+(3-wh)+1)*15)+4+2+15+(wh*30)+(15*k),Disp,ws);
		}
    }
...
//	if(((ch > 31) && (ch < 127))||(ch == 0x0a)){
	if((ch > 31) && (ch < 127)){
		...
		//	if(((sedpoint/ct)>hs)||((sRtnN - sFstN)+1)){
			if((((long)sedpoint/ct)>(hs-1))||((((long)sRtnN - (long)sFstN)+2)>(hs-1))){
				send_scroll(((sRtnPos[sRtnN&0xff]-sRtnPos[sFstN&0xff])/ct)+1);
			}
			sRtnPos[sFstN&0xff] = sstpoint;
		}
		sedpoint++;
	...
	//	if(sRtnN>(hs-1))send_scroll(1);
		if((long)sRtnN>(hs-1))send_scroll(1);
		sRtnN++;
		sedpoint++;
		sRtnPos[sRtnN&0xff] = sedpoint;//新行頭
	}
	key.spnflg = 1;
}
//------------------------------------------------------------------------
...
void HexDataP(HWND hWnd,unsigned char * Bufin,DWORD len)
{
	...
	int RNP = 0;
	int FNP = 0;
	int vsbuf[RTN_SIZE] = {0};
	int vscnt = 0;
	int vsp = 0;

	unsigned int i,j,m,i2,i3,jx;
	unsigned int k,k2,n,n2,n3;
	unsigned int D1,D2,D3,D4;
	unsigned int ws = ((WSIZE_WIDTH-12)/8)-1;
	unsigned int vs = (((((WSIZE_WIDTH-12)/8)-2)/4)*3);//48
	unsigned int vsa = (((((WSIZE_WIDTH-12)/8)-6)/4)*3);//48
	unsigned int hs = ((WSIZE_HEIGHT-80)/75)-1;
	int wh = FIXParam_Cnt/(int)((WSIZE_WIDTH-2)/135);
	if(ws<4)ws=4;
	WCHAR * Disp = new WCHAR[ws+1];
...
//	printf("vs %d hs %d wh %d ws\n",vs,hs,wh,ws);
    ...
    k = 0;
	if(RtnPos[RtnN&0xff] > -1){
		if(RtnPos[FstN&0xff] <= RtnPos[RtnN&0xff]){
			//frmmバッファ端でRtnPos[RtnN]の折り返しが無ければ
			j = RtnPos[RtnN&0xff] - RtnPos[FstN&0xff];//frmmバッファ内の表示文字の量
		}else{
			j = (LINGBUF_SIZE - RtnPos[FstN&0xff]) + RtnPos[RtnN&0xff];
		}
		long base = get_RtnPos_safe(FstN);
		if (base < 0) base = 0;
		for(i=0;i<j;i++){
			char ch = FRMM_at_long(base + i);
			if(ch == 0) k++;//改行指示の空白が含まれているので
		}
	//	if(((j - k)+(len*3)) > (((hs*4)-1)*(vsa))){
	//		scroll(((((j - k)+(len*3))-(((hs*4)-1)*(vsa)))/(vsa))+1);
#if (TEST == 1)
//	if(TESTDBG[8]>0)printf("X %d\n",((((j - k)+(len*3))-(((hs*4)-1)*(vsa)))/(vsa))+1);
#endif
	//	}
		if(RtnPos[FstN&0xff] <= RtnPos[RtnN&0xff]){
			jx = RtnPos[RtnN&0xff] - RtnPos[FstN&0xff];
		}else{
			jx = (LINGBUF_SIZE + RtnPos[RtnN&0xff]) - RtnPos[FstN&0xff];
		}
	}else{
		j = RtnPos[FstN&0xff];//frmmバッファ内の表示文字の量
		if((j+(len*3)) > (((hs*4)-1)*(vsa))){
			scroll((((j+(len*3))/(((hs*4)-1)*(vsa)))/(vsa))+1);
		}
		jx = RtnPos[FstN&0xff];
	}
	...
	k2 = jx/vs;
	if((RtnN - FstN) >= 0){
		if((RtnPos[RtnN&0xff] - RtnPos[FstN&0xff]) >= 0){
			n = RtnPos[RtnN&0xff] - RtnPos[FstN&0xff];
#if (TEST == 1)
	if(TESTDBG[8]>0)printf("A %d %d %d %d %d %d\n",n,k,RtnN,FstN,RtnPos[RtnN],RtnPos[FstN]);
#endif
			if((RtnPos[RtnN&0xff]%(vs+1)) == 0){
				k = vs - ((RtnPos[RtnN&0xff] - RtnPos[FstN&0xff])-(RtnN - FstN))%(vs);
			}else{
				k = vs - ((RtnPos[RtnN&0xff] - RtnPos[FstN&0xff])-(RtnN - FstN - 1))%(vs);
			}
		}else{
			k = vs - (((RtnPos[RtnN&0xff]+LINGBUF_SIZE) - RtnPos[FstN&0xff])-(RtnN - FstN))%(vs+1);
			n = (RtnPos[RtnN]+LINGBUF_SIZE) - RtnPos[FstN];
#if (TEST == 1)
	if(TESTDBG[8]>0)printf("B %d %d %d %d %d %d\n",n,k,RtnN,FstN,RtnPos[RtnN&0xff],RtnPos[FstN&0xff]);
#endif
		}
	}else{
		if((RtnPos[RtnN&0xff] - RtnPos[FstN&0xff]) >= 0){
			k = vs - ((RtnPos[RtnN&0xff] - RtnPos[FstN&0xff])-((RtnN+RTN_SIZE) - FstN))%(vs+1);
			n = RtnPos[RtnN&0xff] - RtnPos[FstN&0xff];
#if (TEST == 1)
	if(TESTDBG[8]>0)printf("C %d %d %d %d %d %d\n",n,k,RtnN,FstN,RtnPos[RtnN&0xff],RtnPos[FstN&0xff]);
#endif
		}else{
			k = vs - (((RtnPos[RtnN&0xff]+LINGBUF_SIZE) 
					- RtnPos[FstN&0xff])-((RtnN+RTN_SIZE) - FstN))%(vs+1);
			n = (RtnPos[RtnN&0xff]+LINGBUF_SIZE) - RtnPos[FstN&0xff];
#if (TEST == 1)
	if(TESTDBG[8]>0)printf("D %d %d %d %d %d %d\n",n,k,RtnN,FstN,RtnPos[RtnN&0xff],RtnPos[FstN&0xff]);
#endif
		}
	}
	D1 = k;
	...
	n2 = 0;
    do {							//追加文字を表示バッファに加える
		j = (RtnPos[RtnN&0xff] + i2);			//左からの文字位置
   	    if(RtnN>0){
			RNP = RtnN-1;
		}else{
			RNP = 0;
		}
   	    if(frm_U[RtnN&0xff] != 1){
			FRMM_set(j, (char)buf[i]);
   	    	i2++;
   	    	i++;
	   	}else{//HEX
			FRMM_set(j, (char)buf[i]);
			...
			if(i2 == k){
//#if (TEST == 1)
//	if(TESTDBG[8]>0)printf("L %d %d %d %d %d %d\n",n,k,RtnN,FstN,RtnPos[RtnN&0xff],RtnPos[FstN&0xff]);
//#endif
				for(m=1;m<(hs*4);m++)RtnPos[(RtnN+m)&0xff] = -1;
				if(n2 != 0){
					RtnN++;
				}else{
					if((RtnPos[RtnN&0xff]%(vs+1)) == 0){
				...
				RtnPos[RtnN&0xff] = j+2;
				i2 = 0;
				k = vs;
				n2++;
//#if (TEST == 1)
//	if(TESTDBG[8]>0)printf("E %d %d %d %d %d %d\n",n,k,RtnN,FstN,RtnPos[RtnN&0xff],RtnPos[FstN&0xff]);
//#endif
			}
		}
	} while (i < (len * 3));
#if (TEST == 1)
	if(TESTDBG[8]>0)printf("F %d %d %d %d %d %d\n",i3,vs,RtnN,FstN,RtnPos[RtnN&0xff],RtnPos[FstN&0xff]);
	if(TESTDBG[8]>0)printf("K %d %d %d %d %d %d\n",RtnPos[0],RtnPos[1],RtnPos[2],RtnPos[3],RtnPos[4],RtnPos[5]);
#endif
	if(i3 != vs){
		j++;
		RtnN++;
	...
	edpoint = j;//文字終端更新
	for(k=1;k<(hs*4);k++)RtnPos[(RtnN+k)&0xff] = -1;
	...
	if(((jx - k)+(len*3)) > (((hs*4)-1)*(vsa))){
		scroll(((((jx - k)+(len*3))-(((hs*4)-1)*(vsa)))/(vsa))+1);
#if (TEST == 1)
	if(TESTDBG[8]>0)printf("X %d\n",((((jx - k)+(len*3))-(((hs*4)-1)*(vsa)))/(vsa))+1);
#endif
	}
	...
	TESTDBG[0] = FstN;
	TESTDBG[1] = RtnN;
	TESTDBG[2] = RtnPos[FstN&0xff];
	TESTDBG[3] = RtnPos[RtnN&0xff];
...
//	TESTDBG[7] = D4;
	...
	if((RtnN&0xff)>3){
	TESTDBG[4] = RtnPos[(RtnN-3)&0xff];
	}else{
	TESTDBG[4] = -1;
	}
	if((RtnN&0xff)>2){
	TESTDBG[5] = RtnPos[(RtnN-2)&0xff];
	}else{
	TESTDBG[5] = -1;
	}
	if((RtnN&0xff)>1){
	TESTDBG[6] = RtnPos[(RtnN-1)&0xff];
	}else{
	TESTDBG[6] = -1;
	}
	TESTDBG[7] = RtnPos[RtnN&0xff];
	if(TESTDBG[8]>0)printf("H %d %d %d %d %d %d\n",RtnPos[0],RtnPos[1],RtnPos[2],RtnPos[3],RtnPos[4],RtnPos[5]);

//	if(RtnN>3){
//	TESTDBG[4] = vsbuf[RtnN-3];
//	}else{
//	TESTDBG[4] = -1;
//	}
//	if(RtnN>2){
//	TESTDBG[5] = vsbuf[RtnN-2];
//	}else{
//	TESTDBG[5] = -1;
//	}
//	if(RtnN>1){
//	TESTDBG[6] = vsbuf[RtnN-1];
//	}else{
//	TESTDBG[6] = -1;
//	}
//	TESTDBG[7] = vsbuf[RtnN];
    ...
    for(k=0;k<(((hs+1)*4)+(3-wh)+2);k++){		//24行
		j = FstN + k;
		long pos = get_RtnPos_safe(j);
		if(frm_U[j&0xff] == 0){//ASCII
			if((pos != -1) && (FRMM_at_long(pos) != 0)){
				const char* source = FRMM_ptr_long(pos);
			   	if (source != nullptr) {
			...
			Disp[ws] = 0;
			TextOutW(hdc,4,(k*15)+4+(wh*30),Disp,ws);
		}else if(frm_U[j&0xff] == 1){//HEX
			n3 = 0;
            if((get_RtnPos_safe(j+1) != -1) && (pos != -1)){
				long next = get_RtnPos_safe(j+1);
				if((next - pos) > 0){
					k2 = (unsigned long)(next - pos);
				}else{
					k2 = (unsigned long)(pos - next);
					if(k2 > (LINGBUF_SIZE/2)){
						k2 = (unsigned long)((next + LINGBUF_SIZE) - pos);
					}
				}
				if(k2 < (unsigned int)vs) k2 = vs;
			}else{
				k2 = vs;
			}
			for(n=0;n<(vs+2+(vs/3));n++)line[n&0xff] = 0;
			for(n=0;n<k2;n++){
				if((pos != -1) && (FRMM_at_long(pos + n) != 0)){
					line[n3&0xff] = FRMM_at_long(pos + n);
					if(n3%3 == 0){
						line[(vs+1+(n3/3))&0xff]
							= Incharset((unsigned char *)FRMM_ptr_long(pos + n));
					} //アスキー表示も右端におまけする
					n3++;
				}
			}
			...
			if(pos != -1 && FRMM_at_long(pos) != 0){
				i = 0;
				Disp[0] = 0;
				while((FRMM_at_long(pos + i) != 0) && (FRMM_at_long(pos + i) != '\n')){
					if((FRMM_at_long(pos + i) > 0x1f) && (FRMM_at_long(pos + i) < 0x7f)){
						/*もし1バイト文字なら*/
						n = (int)(wcslen(Disp));
						if(n<ws){
							Disp[n] = FRMM_at_long(pos + i);
							Disp[n+1] = 0;
						}
						if(i<129)i++;
					}else{
						unsigned char b = (unsigned char)FRMM_at_long(pos + i);
						if((b >= 0xC2) && (b <= 0xDF)){
							n = (int)wcslen(Disp);
							if(n<ws){
								Disp[n] = ((FRMM_at_long(pos + i) & 0x1f) << 6) 
										  |(FRMM_at_long(pos + i + 1) & 0x3f);
								Disp[n+1] = 0;
							}
							if(i<128)i+=2;
							...
							n = (int)wcslen(Disp);
							if(n<ws){
								Disp[n] = ((FRMM_at_long(pos + i) & 0x0f) << 12)
										| ((FRMM_at_long(pos + i + 1) & 0x3f) << 6)
										|  (FRMM_at_long(pos + i + 2) & 0x3f);
								Disp[n+1] = 0;
							}
							if(i<127)i+=3;
							...
							n = (int)wcslen(Disp);
							if(n<ws){
								Disp[n] = ((FRMM_at_long(pos + i) & 0x07) << 18)
										| ((FRMM_at_long(pos + i + 1) & 0x3f) << 12)
										| ((FRMM_at_long(pos + i + 2) & 0x3f) << 6)
										|  (FRMM_at_long(pos + i + 3) & 0x3f);
								Disp[n+1] = 0;
							}
							if(i<126)i+=4;
						}else{
		...
		if(frm_U[j&0xff] == 0){//ASCII
            ...
            if((get_RtnPos_safe(j+1) != -1) && (pos != -1)){
				...
				if((next - pos) > 0){
					...
					n2 = (int)(pos - next);
					if(n2 > (LINGBUF_SIZE/2)){
						n2 = (int)((next + LINGBUF_SIZE)-pos);
					}
				}
				if(n2 < (int)vs) n2 = vs;
			}else{
			...
			if((RtnPos[j&0xff]!=-1)&&(frmm[(RtnPos[j&0xff])&LINGBUF_MAX] != 0)){
				...
				for(j=i;j<ws;j++)Disp[j] = L' ';
				Disp[ws] = 0;
				TextOutW(hdc,4,(k*15)+4+(((unsigned int)wh)*30),Disp,ws);
			}
		}
	}
	p = 0;
    for(k=0;k<(hs+1);k++){			//6行
		...
		if(sRtnPos[j&0xff]!=-1){
			...
			if(sfrm_U[j&0xff] == 1){//hex
				...
				m = 0;
			//	line[0] = 0;
				for(i=0;i<(ws-1);i++)line[i&0xff] = 0;
				for(i=sRtnPos[j&0xff];i<sRtnPos[j&0xff]+p;i++){
    				ltohex((unsigned char)(sfrmm[i&SENDBUF_MAX]),&line[(m*3)],2);
		...
		if(!(strcmp(C_C.text,"DEVICE"))){/*計測器デバイス名 ASCII文字*/
			...
			if(measure_onflg){
				...
				if(!(strcmp(C_C.text,"YOKOKAWA"))){
					MDEV[i].maker = maker_t(MAKER_YOKOKAWA);		//1
				}else if(!(strcmp(C_C.text,"KIKUSUI"))){
					MDEV[i].maker = maker_t(MAKER_KIKUSUI);			//2
				}else if(!(strcmp(C_C.text,"HP"))){
					MDEV[i].maker = maker_t(MAKER_HP);				//3
				}else if(!(strcmp(C_C.text,"KEYSIGHT"))){
					MDEV[i].maker = maker_t(MAKER_KEYSIGHT);		//4
				}else if(!(strcmp(C_C.text,"KEISOKU"))){
					MDEV[i].maker = maker_t(MAKER_KEISOKU);			//5
				}else if(!(strcmp(C_C.text,"FTDI"))){
					MDEV[i].maker = maker_t(MAKER_FTDI);			//6
				}else if(!(strcmp(C_C.text,"OWON"))){
					MDEV[i].maker = maker_t(MAKER_OWON);			//7
				}else if(!(strcmp(C_C.text,"MATSUSADA"))){
					MDEV[i].maker = maker_t(MAKER_MATSUSADA);		//8
				}else if(!(strcmp(C_C.text,"TAKASAGO"))){
					MDEV[i].maker = maker_t(MAKER_TAKASAGO);		//9
				}else if(!(strcmp(C_C.text,"ALIENTEK"))){
					MDEV[i].maker = maker_t(MAKER_ALIENTEK);		//10
				}else if(!(strcmp(C_C.text,"DUMMY"))){
					MDEV[i].maker = maker_t(MAKER_DUMMY);			//0
				}else{
					MDEV[i].maker = maker_t(MAKER_DUMMY);			//0
				}
			}
                     ...
                     int       nCmdShow)
{
	...
	for(i=0;i<5;i++){
		...
		if (!IsHandleValid(hEventObject[i])) {
			// 作成失敗: ログやフォールバックはここで行う（簡潔に NULL に置換）
			hEventObject[i] = NULL;
		}
		...
		if (!IsHandleValid(hEventObjectW[i])) {
			hEventObjectW[i] = NULL;
		}
...
//	MessageBoxW(NULL,Wdummy,L"Debug",MB_ICONHAND);
	...
	lua_pushcfunction(L, SerialClose);
	lua_setglobal(L, "SerialClose");
	...
	lua_pushcfunction(L, SerialClear);
	lua_setglobal(L, "SerialClear");
...
//	MessageBoxW(NULL,_itow(desc.idVendor,Wtmp0,10),_itow(desc.idProduct,Wtmp,10), MB_ICONHAND);
	...
	for(j=0;j<LINGBUF_SIZE;j++)frmm[j&LINGBUF_MAX] = 0x20;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message){
		case WM_CREATE:
		{
			...
			for (i = 0; i < 6; ++i) {
				InitializeCriticalSection(&COMBUF_CS[i]);
			}
			// スレッド実行フラグ初期化（true = 実行許可）
			for (i = 0; i < 10; ++i) {
				gThreadRun[i].store(true);
			}
...
//			lpofn.lStructSize	= OPENFILENAME_SIZE_VERSION_400;
			...
			if((wmId >(0x8000-1))&&(wmId < (0x9000))){
				if(hCom[0] != NULL){
					...
					if(CV[l].flg == 0){
						for(i=0;i<CV[l].len;i++)SND.buf[i&0x3ff]= CV[l].data[i];
						SND.len = CV[l].len;
			...
			}else if((wmId >(0xA000-1))&&(wmId < (0xB000))){
			...
			}else if (wmId >(0xB000-1)){//DirectCMD
				if(SerialRcv_flg == 0){
		...
		case WM_READ_END:/*データ受信*/
		{
			...
			// コピーする長さを安全に取得してロック内でクリア
			EnterCriticalSection(&COMBUF_CS[ch]);
			DWORD bufLen = COMBUF[ch].len;
		...
		//	DeleteObject(hNewFont);
			if(Save_flg)SafeCloseHandle(hSF);   //ファイルを閉じる
		// 1) スレッドに終了要求を送る
			for (i = 0; i < 10; ++i) {
				gThreadRun[i].store(false);
			}
		// 2) スレッドが Wait している可能性のあるイベントをセットしてアンブロック
			for (i = 0; i < 5; ++i) {
				if (COMOverlapped[i].hEvent) SetEvent(COMOverlapped[i].hEvent);
				if (COMOverlappedW[i].hEvent) SetEvent(COMOverlappedW[i].hEvent);
				if (hEventObject[i]) SetEvent(hEventObject[i]);
				if (hEventObjectW[i]) SetEvent(hEventObjectW[i]);
			}
		// 3) スレッドの終了を待ち、ハンドルを閉じる（タイムアウトを設定）
			for (i = 0; i < 10; ++i) {
				if (hThread[i] != NULL) {
		...
		case WM_CHAR:/*キーボードからの直接入力*/
		{
			...
			if(wParam > 31 && wParam < 127){
				if(key_outbuflen < 1024){/*改行までバッファに*/
					SND.buf[(SND.len)&0x3ff] = (unsigned char)wParam;
					SND.len++;
					sfrm_U[sRtnN&0xff] = 0;
				...
				if(SND.len){/*改行で送信*/
					if(COMMPARAM[0].newline == 0){
						SND.buf[(SND.len)&0x3ff] = 0x0a;/*LF*/
						SND.len++;
					}else if(COMMPARAM[0].newline == 1){
						SND.buf[(SND.len)&0x3ff] = 0x0d;/*CRLF*/
						SND.len++;
						SND.buf[(SND.len)&0x3ff] = 0x0a;/*LF*/
						SND.len++;
					}else if(COMMPARAM[0].newline == 2){
...
DWORD WINAPI ThreadFunc(LPVOID vdParam) {
	...
	//	std::ignore =ReadFile(hCom[0],GetBufin[0],131072,&EvParam,&COMOverlapped[0]);
		// Overlapped で ReadFile を発行し、エラーをチェック
			BOOL readRet =ReadFile(hCom[0], GetBufin[0], 131072, &EvParam, &COMOverlapped[0]);
			if (!readRet) {
				DWORD err = GetLastError();
				if (err != ERROR_IO_PENDING) {
					LogOverlappedError("ReadFile ThreadFunc[0]", err);
					SerialRcv_flg = 0;
					Sleep(50); // 過度なループを避ける
					continue;
				}
				// ERROR_IO_PENDING: 正常（完了待ちへ）
			}
		}
		/*ReadFileからのイベントオブジェクトシグナルを調べる*/
		RtnParam = WaitForSingleObject(COMOverlapped[0].hEvent,100);
		if(RtnParam == WAIT_OBJECT_0){
			BOOL got = GetOverlappedResult(hCom[0], &COMOverlapped[0],&len, FALSE);
			if (!got) {
				DWORD err = GetLastError();
				LogOverlappedError("GetOverlappedResult ThreadFunc[0]", err);
			}else if(len > 0){
				SerialRcv_flg = 1;
				len2 = len;
				// 共有バッファ更新はロックで保護
				EnterCriticalSection(&COMBUF_CS[0]);
				...
				COMBUF[0].flg++;
				LeaveCriticalSection(&COMBUF_CS[0]);
				PostMessage(hogeWnd,WM_READ_END,0,0);/*メインスレッドで処理*/
				SerialRcv_flg = 0;
			}
		}else if (RtnParam == WAIT_TIMEOUT) {
			// タイムアウトは通常ループ継続
			SerialRcv_flg = 0;
		}else if (RtnParam == WAIT_FAILED) {
			DWORD err = GetLastError();
			LogOverlappedError("WaitForSingleObject ThreadFunc[0]", err);
			SerialRcv_flg = 0;
			Sleep(50);
		//	FormatMessage(
...
DWORD WINAPI ThreadFunc2(LPVOID vdParam) {
	...
	while (gThreadRun[1].load()) {
		...
		if (!readRet) {
			DWORD err = GetLastError();
			if (err != ERROR_IO_PENDING) {
				LogOverlappedError("ReadFile ThreadFunc[1]", err);
				Sleep(50); // 過度なループを避ける
				continue;
			}
			// ERROR_IO_PENDING: 正常（完了待ちへ）
		...
		if(RtnParam == WAIT_OBJECT_0){
			BOOL got = GetOverlappedResult(hCom[1],&COMOverlapped[1],&len, FALSE);
			if (!got) {
				DWORD err = GetLastError();
				LogOverlappedError("GetOverlappedResult ThreadFunc[1]", err);
			}else if(len > 0){
				len2 = len;
				// 共有バッファ更新はロックで保護
				EnterCriticalSection(&COMBUF_CS[1]);
				if ((COMBUF[1].len + len) < COMMBUFSIZE) {
				...
				if((COMBUF[1].len2+len2)<COMMBUFSIZE){
					for(i=0;i<len2;++i)COMBUF[1].Buf2[COMBUF[1].len2+i]=GetBufin[1][i];
		            COMBUF[1].len2 += len2;
				}else{
				...
				COMBUF[1].flg++;
				LeaveCriticalSection(&COMBUF_CS[1]);
			...
			DWORD err = GetLastError();
			LogOverlappedError("WaitForSingleObject ThreadFunc[1]", err);
			Sleep(50);
		//	FormatMessage(
		//		FORMAT_MESSAGE_ALLOCATE_BUFFER |
		//		FORMAT_MESSAGE_FROM_SYSTEM,
...
DWORD WINAPI ThreadFunc3(LPVOID vdParam) {
	...
	while (gThreadRun[2].load()) {
		...
		if (!readRet) {
			DWORD err = GetLastError();
			if (err != ERROR_IO_PENDING) {
				LogOverlappedError("ReadFile ThreadFunc[2]", err);
				Sleep(50); // 過度なループを避ける
				continue;
			}
			// ERROR_IO_PENDING: 正常（完了待ちへ）
		...
		if(RtnParam == WAIT_OBJECT_0){
			BOOL got = GetOverlappedResult(hCom[2],&COMOverlapped[2],&len, FALSE);
			if (!got) {
				DWORD err = GetLastError();
				LogOverlappedError("GetOverlappedResult ThreadFunc[2]", err);
			}else if(len > 0){
				len2 = len;
				// 共有バッファ更新はロックで保護
				EnterCriticalSection(&COMBUF_CS[2]);
				if ((COMBUF[2].len + len) < COMMBUFSIZE) {
				...
				if((COMBUF[2].len2+len2)<COMMBUFSIZE){
					for(i=0;i<len2;++i)COMBUF[2].Buf2[COMBUF[2].len2+i]=GetBufin[2][i];
		            COMBUF[2].len2 += len2;
				}else{
				...
				COMBUF[2].flg++;
				LeaveCriticalSection(&COMBUF_CS[2]);
			...
			DWORD err = GetLastError();
			LogOverlappedError("WaitForSingleObject ThreadFunc[2]", err);
			Sleep(50);
		//	FormatMessage(
		//		FORMAT_MESSAGE_ALLOCATE_BUFFER |
		//		FORMAT_MESSAGE_FROM_SYSTEM,
...
DWORD WINAPI ThreadFunc4(LPVOID vdParam) {
	...
	while (gThreadRun[3].load()) {
		...
		if (!readRet) {
			DWORD err = GetLastError();
			if (err != ERROR_IO_PENDING) {
				LogOverlappedError("ReadFile ThreadFunc[3]", err);
				Sleep(50); // 過度なループを避ける
				continue;
			}
			// ERROR_IO_PENDING: 正常（完了待ちへ）
		...
		if(RtnParam == WAIT_OBJECT_0){
			BOOL got = GetOverlappedResult(hCom[3],&COMOverlapped[3],&len, FALSE);
			if (!got) {
				DWORD err = GetLastError();
				LogOverlappedError("GetOverlappedResult ThreadFunc[3]", err);
			}else if(len > 0){
				len2 = len;
				// 共有バッファ更新はロックで保護
				EnterCriticalSection(&COMBUF_CS[3]);
				if ((COMBUF[3].len + len) < COMMBUFSIZE) {
				...
				if((COMBUF[3].len2+len2)<COMMBUFSIZE){
					for(i=0;i<len2;++i)COMBUF[3].Buf2[COMBUF[3].len2+i]=GetBufin[3][i];
		            COMBUF[3].len2 += len2;
				}else{
				...
				COMBUF[3].flg++;
				LeaveCriticalSection(&COMBUF_CS[3]);
			...
			DWORD err = GetLastError();
			LogOverlappedError("WaitForSingleObject ThreadFunc[3]", err);
			Sleep(50);
		//	FormatMessage(
		//		FORMAT_MESSAGE_ALLOCATE_BUFFER |
		//		FORMAT_MESSAGE_FROM_SYSTEM,
...
DWORD WINAPI ThreadFunc5(LPVOID vdParam) {
	...
	while (gThreadRun[4].load()) {
		...
		if (!readRet) {
			DWORD err = GetLastError();
			if (err != ERROR_IO_PENDING) {
				LogOverlappedError("ReadFile ThreadFunc[4]", err);
				Sleep(50); // 過度なループを避ける
				continue;
			}
			// ERROR_IO_PENDING: 正常（完了待ちへ）
		...
		if(RtnParam == WAIT_OBJECT_0){
			BOOL got = GetOverlappedResult(hCom[4],&COMOverlapped[4],&len, FALSE);
			if (!got) {
				DWORD err = GetLastError();
				LogOverlappedError("GetOverlappedResult ThreadFunc[4]", err);
			}else if(len > 0){
				len2 = len;
				// 共有バッファ更新はロックで保護
				EnterCriticalSection(&COMBUF_CS[4]);
				if ((COMBUF[4].len + len) < COMMBUFSIZE) {
			...
			size_t space2=(COMMBUFSIZE>COMBUF[4].len2)?(COMMBUFSIZE-(COMBUF[4].len2)):0;
				...
				COMBUF[4].flg++;
				LeaveCriticalSection(&COMBUF_CS[4]);
			...
			DWORD err = GetLastError();
			LogOverlappedError("WaitForSingleObject ThreadFunc[4]", err);
			Sleep(50);
		//	FormatMessage(
		//		FORMAT_MESSAGE_ALLOCATE_BUFFER |
		//		FORMAT_MESSAGE_FROM_SYSTEM,
